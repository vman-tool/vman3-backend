...
async def get_submissions_statistics(
    ...
    response = await fetch_submissions_statistics(paging=allow_paging, page_number=page_number, limit=limit, start_date=start_date, end_date=end_date, locations=locations,date_type=date_type, db=db)
    return response
...
async def get_charts_statistics(
    ...
    allow_paging = False if paging is not None and paging.lower() == 'false' else True
    response = await fetch_charts_statistics(paging=allow_paging, page_number=page_number, limit=limit, start_date=start_date, end_date=end_date, locations=locations,date_type=date_type, db=db)
    return response
